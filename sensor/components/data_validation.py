...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        """
        try:
            threshold = self.data_validation_config.missing_thresholed
            null_report = df.isna().sum()/df.shape[0] # this will give values is persentage
        ...
        try:
            ...
            for col in base_columns:
                if col not in current_columns:
                    logging.info(f"Column:[{col} is not avaliable.]")
    ...
    def data_drift(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str):
        try:
            ...
            for col in base_columns:
                ...
                logging.info(f"Hypothesis {col}: {base_data.dtypes}, {current_data.dtypes} ")
                same_distribution = ks_2samp( base_data,current_data)

                if same_distribution.pvalue > 0.05:
                    #we are accepting the null hypo 
                    drift_report[col] = {
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        ...
        try:
            ...
            write_yaml_file(file_path=self.data_validation_config.report_file,
            data=self.validation_error)

            data_validation_artifact = artifact_entity.DataValidationArtifact(report_file_path=self.data_validation_config.report_file,)
